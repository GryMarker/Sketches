/*
  XIAO ESP32S3 Sense (OV3660) — STA camera + VL53L7CX (8x8) with LPn on D5
  Endpoints:
    /jpg     : single JPEG (Android app polls this)
    /plain   : simple page auto-refreshing /jpg
    /health  : JSON heap/psram/uptime
    /grid    : JSON[64] distances (mm), rotated 90° CW to match camera (board on right side)
...
#include <SparkFun_VL53L5CX_Library.h>   // Install: "SparkFun VL53L5CX Arduino Library"
SparkFun_VL53L5CX tof;
VL53L5CX_ResultsData tofData;
bool tof_ok = false;

// ====== ToF wiring/orientation ======
#define TOF_LPN_PIN   8          // your LPn on D5
// Rotation of sensor relative to camera: 0, 90, 180, 270 (clockwise)
#define TOF_ROTATION  90         // you said "rotated clockwise 1 time"
...
// ====== Wi-Fi (STA) ======
static const char* WIFI_SSID = "Chalatungs Intergalactic WIFI";
static const char* WIFI_PASS = "PLDTWIFIz86fu*";
...
bool startCamera(){
  ...
  c.frame_size   = FRAMESIZE_QVGA; // 320x240 works well with the app
...
// ---------- ToF ----------
bool startToF(){
  // Handle LPn (hardware shutdown) on D5
  pinMode(TOF_LPN_PIN, OUTPUT);
  digitalWrite(TOF_LPN_PIN, LOW);   // keep in reset
  delay(2);
  digitalWrite(TOF_LPN_PIN, HIGH);  // power up
  delay(3);

  // I2C on XIAO: SDA=6, SCL=7
  Wire.begin(6, 7);
